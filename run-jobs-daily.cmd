...
set "CMD_LOG_PATH=D:\SPA\Python\logs\cmd_execution_daily.log"
...
REM Inicio del log
echo Inizio esecuzione daily jobs: %date% %time% > "%CMD_LOG_PATH%"

REM Ejecución del script Python con parámetro 1 (daily)
"%PYTHON_PATH%" "%SCRIPT_PATH%" 1 >> "%CMD_LOG_PATH%" 2>&1
...
echo Fine esecuzione daily jobs: %date% %time% >> "%CMD_LOG_PATH%"
